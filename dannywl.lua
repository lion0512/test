...
splashFrame.Size = UDim2.new(0, 350, 0, 180)
splashFrame.Position = UDim2.new(0.5, -175, 0.5, -90)
...
splashLabel.Position = UDim2.new(0, 0, 0, 30)
...
barBackground.Position = UDim2.new(0.5, -150, 0, 100)
...
-- Status Text (shows "Loading..." then "Done")
...
-- Animate loading bar over 17 seconds
spawn(function()
    local totalDuration = 17
    local updateInterval = 0.05
    local steps = totalDuration / updateInterval
    local stepSize = 1 / steps
    
    for i = 1, steps do
        barFill.Size = UDim2.new(i * stepSize, 0, 1, 0)
        wait(updateInterval)
    end
    ...
    tweenService:Create(splashFrame, tweenInfo, fadeGoal):Play()
    tweenService:Create(splashLabel, tweenInfo, textFadeGoal):Play()
    tweenService:Create(statusLabel, tweenInfo, textFadeGoal):Play()
    tweenService:Create(barBackground, tweenInfo, fadeGoal):Play()
    tweenService:Create(barFill, tweenInfo, fadeGoal):Play()
    wait(0.5)
    splash:Destroy()
end)

-- Main GUI (appears after splash is gone)
...
-- Show main GUI after splash is destroyed (approximately 18 seconds)
spawn(function()
    wait(18.5)
    frame.Visible = true
end)
