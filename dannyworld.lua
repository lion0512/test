...
espHighlightFolder.Parent = game.CoreGui

-- Common item keywords to search for in Danny World
local itemKeywords = {
    "item", "key", "tool", "gear", "battery", "fuse", "lever",
    "crank", "valve", "tape", "medkit", "bandage", "potion",
    "scroll", "gem", "crystal", "part", "fragment", "core",
    "collect", "pickup", "objective", "quest"
}
...
local function isItem(obj)
    ...
    -- Also check if it has a ProximityPrompt or ClickDetector (interactive objects)
...
local function createHighlight(obj)
    if espHighlightFolder:FindFirstChild(obj.Name .. "_ESP") then return end
    ...
    -- BillboardGui for distance and name
    local billboard = Instance.new("BillboardGui")
    billboard.Name = obj.Name .. "_Billboard"
    billboard.Parent = espHighlightFolder
    ...
    for _, descendant in pairs(workspace:GetDescendants()) do
        if descendant:IsA("BasePart") or descendant:IsA("Model") or descendant:IsA("Tool") then
            if isItem(descendant) then
                local part = descendant:IsA("BasePart") and descendant or descendant:FindFirstChildWhichIsA("BasePart")
                if part then
                    createHighlight(part.Parent or part)
...
espButton.MouseButton1Click:Connect(function()
    ...
    if espEnabled then
        scanForItems()
        -- Rescan every 5 seconds for new items
        spawn(function()
            while espEnabled do
                wait(5)
                scanForItems()
